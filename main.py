...
def main():
    ...
    llm=ChatOllama(model="qwen3:4b", verbose=True, temperature=0.0, num_predict=2000)
    #llm=ChatOllama(model="gpt-oss:20b", verbose=True, temperature=0.0, num_predict=2000)

    chain=summary_prompt_template | llm 
    '''
        this uses the first argument and uses the output of the argument as the input
        for the second argument. So is like a 'chain' where we can define multiple 
        steps in order to get a much precise response. 
    '''
